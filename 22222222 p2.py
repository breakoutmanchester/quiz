...
ser3 = serial.Serial('COM19', 9600)  
ser3.timeout = 1

ser4 = serial.Serial('COM21', 9600)  
ser4.timeout = 1
# Initialize Pygame
pygame.init()
    ...
    while not answer_received and attempts < 1:  # Allow up to 3 attempts per question
        for ser in [ser2, ser4]:
            if ser.in_waiting > 0:
                ...
                # Check if data is a number, operator or equals signal
                if serial_data.isdigit():
                    print(f"Received number: {serial_data}")  # Debug message

                    if operator:
                        sum_string += " " + operator + " "
                        print(f"Operator: {operator}")  # Debug message

                        if operator == '+':
                            sum_value += int(serial_data)
                        elif operator == '-':
                            sum_value -= int(serial_data)
                        elif operator == 'x':
                            sum_value *= int(serial_data)
                    ...
                    sum_string += serial_data  # Append number to sum string
                    window.blit(sum_text, (position_x, position_y))
                    pygame.display.flip()
                    print(f"Current sum string: {sum_string}")  # Debug message

# Display sum string on screen
                    window.blit(current_question_surface, (0, 0))  # Fill the screen with black to clear previous text

                    sum_text = font.render('Sum: {}'.format(sum_string), True, (255, 53, 53))  # Now the text is white

                    # Calculate the position for the text to be centered
                    text_width, text_height = sum_text.get_size()
                    position_x = (WINDOW_WIDTH - text_width) / 2
                    position_y = (WINDOW_HEIGHT - text_height) / 1.8

                    # Blit the text surface at the calculated position
                    window.blit(sum_text, (position_x, position_y))
                    pygame.display.flip()

                elif serial_data in ['+', '-', 'x', '/']:
                    operator = serial_data  # Set current operator
                    sum_string += " " + serial_data + " "
                    print(f"Received operator: {operator}")  # Debug message

                elif serial_data == 'E':
                    print(f"Received equals signal")  # Debug message
                    sum_string += " = " + str(sum_value)  # Append equals sign and sum to sum string
                    print(f"Final sum string: {sum_string}")  # Debug message
                    print(f"Sum value: {sum_value}")  # Debug message

                    # Display sum string on screen
                    # In the main while loop, replace the existing 'window.blit(sum_text, ...' line with the following:

                    # Display sum string on screen
                    window.blit(current_question_surface, (0, 0))  # Fill the screen with black to clear previous text

                    sum_text = font.render('{}'.format(sum_string), True, (255, 53, 53))  # Now the text is white 
                    ...
                    if str(sum_value) == quiz_answers[i]:
                        print('Correct!')
                        feedback_surface = correct_surface
                        answer_received = True
                        score += 1  # Add 1 to score for correct answer
                        if str(sum_value) == '125':
                            score += 4
                    ...
                    sum_value = 0  # Reset sum
                    operator = None  # Reset operator
                    sum_string = ""  # Reset sum string
